<?php
	...
	class Image {
		...
		public function upload_img(){
			...
			if (!isset($this->name)) {
				$error = "Veuillez mettre une image !";
				header('Location:' . CURRENT_URL ."?error=".$error);
				exit;
			}
			...
			if($width_orig < 500 && $height_orig < 500 && $width_orig > 6000 && $height_orig > 6000){
				$error = "Dimension de l'image minimum 400 x 400 et maximum 6000 x 6000 !";
				header('Location:' . CURRENT_URL ."?error=".$error);
				exit;
			}
			...
			if ($this->size > $tailleMax){ 
				$error = "Votre photo de profil ne doit pas dépasser 5 Mo !";	        
				header('Location:' . CURRENT_URL ."?error=".$error);
				exit;
			}
			...
			if (!in_array($extensionUpload, $extensionsValides)){ // Vérifie que l'extension est correct
				$error = "Votre photo doit être au format jpg, jpeg ou png.";
				header('Location:' . CURRENT_URL ."?error=".$error);
			}	    						    	    
			...
			if (!$resultat){ 
				$error = "Erreur lors de l'importation de votre photo.";
				header('Location:' . CURRENT_URL ."?error=".$error);
				exit;
			}
			...
			if (!is_readable($chemin)){
				$error = "Le type MIME de l'image n'est pas bon";
				header('Location:' . CURRENT_URL ."?error=".$error);
				exit;
			}
			...
			if($this->type != $ListeExtension[$extensionUpload]  && $this->type != $ListeExtensionIE[$extensionUpload]){	
				$error = "L'extension n'est pas ok";
				header('Location:' . CURRENT_URL ."?error=".$error);
				exit;
			}
			...
			$error = "Nouvel image dans l'album !";
			header('Location:' . CURRENT_URL ."?error=".$error);
			exit;
		}
		...
		public function delete_img($p_NameImg){
			...
			if(!isset($p_NameImg)){
				$error = "Il n'y a pas d'image a supprimer";
				header('Location:' . CURRENT_URL ."?error=".$error);
				exit;
			}
	    	...
	    	if(!isset($imageBD['id'])){
	    		$error = "L'image a déjà été supprimé";
				header('Location:' . CURRENT_URL ."?error=".$error);
				exit;
	    	}
			...
			$error = "Votre image a été supprimé de l'album !";
			header('Location:' . CURRENT_URL ."?error=".$error);
			exit;
